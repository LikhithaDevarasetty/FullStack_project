...
def login():
    if request.method == 'POST':
        # Allow login with username OR email to be more user friendly
...
if __name__ == '__main__':
    with app.app_context():
        # Create tables if they don't exist. Do NOT drop_all() here because that
        # would erase persisted users and bookings on every server restart.
        db.create_all()
        # Seed sample packages only if there aren't any yet.
